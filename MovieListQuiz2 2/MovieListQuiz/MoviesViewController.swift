...
class MoviesViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let movieListN = movies[indexPath.row]
        let movieTitle = movieListN.title
        let movieDirector = movieListN.director
        let bringMovieImage = movieListN.image
        let movieImage = UIImage(named: bringMovieImage)
        
        cell.textLabel?.text = "\(movieTitle)"
        cell.detailTextLabel?.text = "\(movieDirector)"
